...
Reusable patterns for thread-pool parallelism:

- :class:`ParallelStepGroup` -- run independent named callables concurrently
- :class:`ParallelItemProcessor` -- map a function over items in parallel
- :class:`ChunkedParallelProcessor` -- process a DataFrame in parallel chunks
- :class:`ProgressTracker` -- thread-safe progress with optional rendering
"""
...
class ParallelStepGroup:
    ...
    Usage::
        ...
        group = ParallelStepGroup(tasks=[
            ...
            ("rosters", build_rosters, {"season": "2024"}),
        ])
        results = group.execute()
        results["ratings"].success, results["rosters"].result
    """
...
class ParallelItemProcessor:
    """Process a list of items in parallel with progress reporting.

    Usage::

        processor = ParallelItemProcessor(
            items=seasons,
            process_fn=pull_season,
            max_workers=4,
        )
...
class ChunkedParallelProcessor:
    ...
    Requires pandas (not a core dependency of this package -- install it
    yourself or via the ``dataframe`` extra).
    ...
    def execute(self):
        try:
            ...
            raise ImportError(
                "ChunkedParallelProcessor requires pandas: pip install pandas"
            ) from e
